...
export class EmpleadoList2Component implements OnInit {
  ...
  confirmarEliminarEmpleado(idEmpleado: 0): void {
    ...
    if (empleado) {
      const confirmacion = window.confirm(`¿Estás seguro de que quieres eliminar al empleado: ${empleado.nombre}?`);
      if (confirmacion) {
  ...
  eliminarEmpleado(idEmpleado: 0): void {
    this.empleadoServicio.DeleteEmpleado(idEmpleado.toString())
      .subscribe({
